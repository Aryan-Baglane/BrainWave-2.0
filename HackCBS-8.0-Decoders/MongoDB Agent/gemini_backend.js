...
if (!geminiApiKey || !openRouterApiKey || !mongoUri) {
  console.error("Missing GEMINI_API_KEY, OPENROUTER_API_KEY, or MONGODB_URI in .env file.");
  process.exit(1);
}

// Gemini Model Configuration
const chatModel = 'nvidia/nemotron-3-nano-30b-a3b:free'; // Matched your curl command
...
async function callGemini(systemPrompt, userQuery, apiKey, isJson = true) {
  ...
  // Note: response_format json_object requires the model to support it. 
  // Gemini 2.0 Flash via OpenRouter might support it or not.
  // Safest to just prompt for JSON and parse.
  // Removing response_format if it causes issues, but leaving matches OpenAI spec.
  // Actually, let's trust the system prompt + parsing.
...
async function runNlToQueryPipeline(userInput, collectionName, user) {
  ...
  const mongoQuery = await callGemini(QUERY_AGENT_PROMPT, userInput, openRouterApiKey);

  // 2. Security Agent
  console.log('[NL-to-Query] Calling Security Agent...');
  const securityCheck = await callGemini(SECURITY_AGENT_PROMPT, JSON.stringify(mongoQuery), openRouterApiKey);
  ...
  const optimizedQuery = await callGemini(OPTIMIZATION_AGENT_PROMPT, JSON.stringify(mongoQuery), openRouterApiKey);

  // 4. Validation Agent
  console.log('[NL-to-Query] Calling Validation Agent...');
  const validation = await callGemini(VALIDATION_AGENT_PROMPT, userInput, openRouterApiKey);
...
async function runRagPipeline(userInput, user) {
  ...
  const queryVector = await getEmbedding(userInput, geminiApiKey);
...
async function runUpdatePipeline(userInput, collectionName, user) {
  ...
  const updateQuery = await callGemini(UPDATE_AGENT_PROMPT, userInput, openRouterApiKey);

  // 3. Update Security Agent
  console.log('[Update] Calling Update Security Agent...');
  const securityCheck = await callGemini(UPDATE_SECURITY_AGENT_PROMPT, JSON.stringify(updateQuery), openRouterApiKey);
  if (!securityCheck.isSafe) {
  ...
  if (updateResult.modifiedCount > 0) {
    ...
    for (const doc of updatedDocs) {
      ...
      const embedding = await getEmbedding(textToEmbed, geminiApiKey);
